...
pub trait Model: BlockingModel + Send + Sync + 'static {
    ...
    async fn insert_many(docs: &[Self]) -> Result<InsertManyResult> {
        ...
        Ok(spawn_blocking(move || Self::collection().insert_many(docs, None)).await??)
    }
        ...
        Q: Into<Document> + Send + 'static,
    {
        ...
        Ok(
            spawn_blocking(move || Self::collection().update_many(query.into(), update, None))
                .await??,
        )
    }
    ...
    async fn remove(&self) -> Result<DeleteResult> {
        let query = doc! {"_id": self.id()};
        Ok(spawn_blocking(move || Self::collection().delete_one(query, None)).await??)
    }
}
